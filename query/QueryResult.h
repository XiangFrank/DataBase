...
class SelectQueryResult : public SuceededQueryResult {
    std::string msg;
public:
    bool display() override { return true; }

    explicit SelectQueryResult (std::vector<std::string> results) {
...
class AnswerQueryResult : public SuceededQueryResult {
    std::string msg;
public:
    bool display() override { return true; }

    explicit AnswerQueryResult(const int number) {
        this->msg = R"(ANSWER = ?)"_f % number;
    }

    explicit AnswerQueryResult(std::vector<int> results) {
        std::stringstream ss;
        ss << "ANSWER = ( ";
        for (auto result : results) {
            ss << result << " ";
