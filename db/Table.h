...
public:
    ...
    public:
        ...
        void Remap(SizeType reduce){
            table->keyMap[it->key] -= reduce;
        }
...
public:
    ...
    //Xiang
    bool checkByIndex(KeyType key);

    void deleteKeyMap(KeyType key);
    ...
    int erase_x(Iterator begin, Iterator end){
        //Datum temp;
        //temp.mark = 1;
        auto it_temp = std::remove(begin.it, end.it, 1);
        int CONT = 0;
        int cnt = 0;
        for(auto it = it_temp; it != end.it; it++) {
            this->keyMap.erase(it->key);
            CONT ++;
        }
        for(auto it = it_temp; it != end.it; it++) {
            this->data.erase(it);
        ...
        //this->data.erase(it_temp, end.it);
        if(cnt == CONT){
            printf("fuck");
        }
        return CONT;
