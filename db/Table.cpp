...
//Xiang
bool Table::checkByIndex(KeyType key){
    return this->keyMap.find(key) != this->keyMap.end();
}

void Table::deleteKeyMap(KeyType key){
    auto it = this->keyMap.find(key);
    this->keyMap.erase(it);
}

int Table::erase_mark() {
    auto it_temp = std::remove(this->data.begin(), this->data.end(), 1);
    int COUNT = 0;
    while(it_temp != this->data.end()) {
        this->data.pop_back();
        COUNT ++;
    }
    return COUNT;
