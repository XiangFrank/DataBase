...
Table &Database::CopyTable(std::string TableName, std::string OldTableName) {
    auto &db = Database::getInstance();

    auto fields = db[OldTableName].field();
    auto old_table = db[OldTableName];
    auto table = std::make_unique<Table>(TableName, fields);

    std::vector<Table::KeyType> keys;
    for (auto it = old_table.begin(); it != old_table.end(); ++it) {
        Table::KeyType key = (*it).key() + "";
        std::vector<Table::ValueType> tuple;
        Table::SizeType fieldCount = old_table.field().size();
        tuple.reserve(fieldCount - 1);
        for (Table::SizeType i = 0; i < fieldCount; i++) {
